...
with st.expander("Com s'han de pujar els informes oficials?", expanded=True):
    st.markdown(
        """
        - Si l'informe oficial de notes és **un únic PDF per alumne**, puja aquests PDFs només a **Informe oficial - part 1**.
        - Si l'informe oficial surt dividit en **dos PDFs per alumne**, separa abans els arxius en dues carpetes al teu ordinador:
        ...
        - No barregis la part 1 i la part 2 en el mateix carregador, perquè l'aplicació no pot saber amb seguretat quin PDF ha d'anar primer.
        - El PSI i les observacions són opcionals: només s'afegiran als alumnes que tinguin un PDF corresponent.
        """
    )
...
with col2:
    ...
    pdf_informe_1 = st.file_uploader(
        ...
        help="Sempre s'ha d'omplir aquest apartat. Si l'informe oficial només té un PDF per alumne, puja'l aquí.",
    )
    pdf_informe_2 = st.file_uploader(
        "Informe oficial - part 2, només si existeix",
        type=["pdf"],
        accept_multiple_files=True,
        help="Només s'ha d'omplir si l'informe oficial de notes surt dividit en dos PDFs per alumne.",
    )
...
st.info(
    "La coincidència es fa pel nom de l'arxiu. L'aplicació prioritza les coincidències completes, "
